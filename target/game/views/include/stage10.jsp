...
<ol>
    ...
    <li>Выберите наиболее правильное из утверждений:
        <ul>
            ...
            <li onclick="selectInput(this)"><input type="radio" name="stage10_q<%=i%>" value="1">&nbsp;<%=phrase%></li>
            <% phrase = model.getPhrase(i, 2);
                slonomuh = Stage10Model.PHRASES.getNumberByText(phrase);
                player.setSlonomuh(i-1, 1, slonomuh);%>
            <li onclick="selectInput(this)"><input type="radio" name="stage10_q<%=i%>" value="2">&nbsp;<%=phrase%></li>
            <% phrase = model.getPhrase(i, 3);
                slonomuh = Stage10Model.PHRASES.getNumberByText(phrase);
                player.setSlonomuh(i-1, 2, slonomuh);%>
            <li onclick="selectInput(this)"><input type="radio" name="stage10_q<%=i%>" value="3">&nbsp;<%=phrase%></li>
            <% phrase = model.getPhrase(i, 4);
                slonomuh = Stage10Model.PHRASES.getNumberByText(phrase);
                player.setSlonomuh(i-1, 3, slonomuh);%>
            <li onclick="selectInput(this)"><input type="radio" name="stage10_q<%=i%>" value="4">&nbsp;<%=phrase%></li>
        </ul>
        <div id="stage10_warning<%=i%>" class="red" style='display:none;'>Сделайте выбор</div>
    </li>
    <br>
    <% } %>
</ol>